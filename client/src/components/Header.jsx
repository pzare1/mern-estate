  ...
  return (
    <header className='bg-slate-100 shadow-sm'>
      <div className='flex justify-between align-middle items-center max-w-6xl mx-auto p-3'>
        ...
        <input className=' bg-transparent px-2 py-1 focus:outline-none w-28 sm:w-80 placeholder:text-sm text-slate-600' placeholder='Search ...'></input>
        <CiSearch className='mx-2 text-slate-400'/>
        </form>
        <ul className='flex justify-between text-slate-600 text-sm'>
          <Link to='/'>
          <li className='hidden sm:inline px-3 cursor-pointer'>Home</li>
          </Link>
          <Link to='/About'>
          <li className='hidden sm:inline px-3 cursor-pointer'>Services</li>
          </Link>
          <Link to='/login'>
